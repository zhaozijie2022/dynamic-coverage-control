# 一共有两个类 Pd 和 Pdtype, 其中Pdtype可以通过pdclass调用Pd类,
# 并使用 flat参数 将Pd类初始化为一个具体的对象, flat为该分布的核心参数,
# 比如分类分布的独热向量, 高斯分布的均值方差等
...
# 继承object类是Python2中的传统, Python3中默认继承object类
class Pd(object):
    """
...
class PdType(object):
    ...
    def pdfromflat(self, flat):
        ...
        # self.pdclass()返回一个Pd类, (flat)将该类初始化为对象, flat为各Pd类的初始化参数
...
class CategoricalPdType(PdType):
    """分类分布, 独热(One-hot)向量, Categorical直译为绝对的. 例子: 1个球仍k个桶"""
    def __init__(self, ncat):  # ncat = num_actions, 可采取的动作数, 即动作空间的维数
        self.ncat = ncat
    def pdclass(self):
        return CategoricalPd  # 注意, 这里是返回了一个类, 不是将这个类初始化为一个对象
    def param_shape(self):
        return [self.ncat]
    def sample_shape(self):
        return []
    def sample_dtype(self):  # 数据类型
        return tf.int32
...
class CategoricalPd(Pd):
    ...
    def flatparam(self):
        return self.logits
    def mode(self):
        return U.argmax(self.logits, axis=1)  # 返回最大索引, 即每一行的独热位置, 每一行为一个样本, 即每个样本的类别
    def logp(self, x):
        return -tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits, labels=x)
        # 计算 logits 和 labels 之间的稀疏 softmax 交叉熵
        # label是一个直接存储类别向量[0 1 5]而不是与logits同型的独热稀疏矩阵
    def kl(self, other):
...
class SoftCategoricalPd(Pd):
    ...
    def entropy(self):
        ...
        return U.sum(p0 * (tf.log(z0) - a0), axis=1)
    def sample(self):
        u = tf.random.uniform(tf.shape(self.logits))  # 增加均匀噪声, 使得采样具有随机性
...
def make_pdtype(ac_space):
    """判断传入的ac_space是那种类型, 返回对应的分布(均在distributions.py中定义的类)"""
    from gym import spaces

    # isinstance函数判断两个参数是否属于同一类, **它将子类与父类认为是同一类
    if isinstance(ac_space, spaces.Box):
        assert len(ac_space.shape) == 1  # assert 检查后续的条件, 不符合就终止程序
        # 动作空间只能是一维向量
        return DiagGaussianPdType(ac_space.shape[0])  # ac_space.shape[0]动作空间大小, 即可采取的动作数量
    ...
    elif isinstance(ac_space, spaces.MultiBinary):  # 多维二进制--伯努利分布
